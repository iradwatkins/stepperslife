...
// DEVELOPMENT Convex URL (from .env.local)
const CONVEX_URL = "https://quiet-robin-620.convex.cloud";
...
async function createEvents() {
  console.log("🎉 Creating events in DEVELOPMENT database...");
  ...
  try {
    ...
    // Verify all events
    console.log("\n✅ All events created successfully in DEVELOPMENT database!");
    
    const events = await client.query(api.events.get, {});
    console.log(`\n📊 Total events in development database: ${events.length}`);
    console.log("\n📋 Events list:");
    ...
    console.log("\n🌐 View events locally at: http://localhost:3000");
    console.log("🌐 Or on production at: https://stepperslife.com (after config update)");
