...
export const cleanupExpiredOffers = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    for (const [eventId, offers] of Object.entries(grouped)) {
      ...
      await ctx.runMutation(internal.waitingList.processQueueInternal, { eventId: eventId as Id<"events"> });
    }
  },
});
...
/**
 * Internal version of processQueue for use in other mutations
 */
export const processQueueInternal = internalMutation({
  args: {
    eventId: v.id("events"),
  },
  handler: async (ctx, { eventId }) => {
