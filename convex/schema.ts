...
export default defineSchema({
  ...
  // Event staff (scanners and managers) authorization table
  eventStaff: defineTable({
    eventId: v.id("events"),
    userId: v.string(), // The user who has permission
    email: v.string(), // Email of the staff member
    role: v.union(
      v.literal("scanner"), // Can scan tickets
      v.literal("manager"), // Can scan tickets and manage other scanners
      v.literal("organizer") // Full control (event owner)
    ),
    
    // Invitation tracking
    invitedBy: v.string(), // User ID of who invited them
    invitedAt: v.number(),
    acceptedAt: v.optional(v.number()),
    invitationToken: v.optional(v.string()), // For email invitation links
    ...
    canScan: v.boolean(), // Can scan QR codes
    canManageStaff: v.boolean(), // Can add/remove other staff
    canViewReports: v.boolean(), // Can view event reports
    maxScansPerHour: v.optional(v.number()), // Rate limiting
    ...
    .index("by_status", ["invitationStatus", "isActive"]),
  
  // Scan logs for audit trail
  scanLogs: defineTable({
    eventId: v.id("events"),
    ticketId: v.id("simpleTickets"),
    scannedBy: v.string(), // User ID of scanner
    ...
    .index("by_ticket", ["ticketId"])
    .index("by_scanner", ["scannedBy"])
    .index("by_timestamp", ["scanTimestamp"])
    .index("by_event_scanner", ["eventId", "scannedBy"]),
